    ...
    @else
        <form wire:submit="submit" class="mt-8 grid min-w-0 gap-8 lg:grid-cols-[minmax(0,1fr)_20rem]">
                ...
                <section class="rounded-panel border border-line bg-surface p-5 sm:p-7">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <select id="vehicleId" wire:model.live="vehicleId" class="mt-2 min-h-11 w-full rounded-field border border-line-strong bg-surface px-3.5 text-sm focus:border-brand focus:ring-2 focus:ring-brand-soft">
                                <option value="">Pilih kendaraan</option>
                                @foreach ($vehicles as $option)
                                    <option value="{{ $option->id }}">{{ $option->name }} — {{ $option->plate_number }}</option>
                                @endforeach
                            </select>
                            @error('vehicleId') <p class="mt-1.5 text-sm text-danger">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </section>
                ...
                <section class="rounded-panel border border-line bg-surface p-5 sm:p-7">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <input id="booking-date" type="date" wire:model.live="date" min="{{ now()->toDateString() }}" class="mt-2 min-h-11 w-full rounded-field border border-line-strong bg-surface px-3.5 text-sm sm:max-w-xs">
                            @error('date') <p class="mt-1.5 text-sm text-danger">{{ $message }}</p> @enderror

                            @if ($calculation)
                                <p class="mt-3 flex items-start gap-2 text-sm text-ink-muted">
                                    <x-heroicon-o-sparkles class="mt-0.5 size-4 shrink-0 text-brand" />
                            ...
                            <div class="mt-6" wire:loading.class="opacity-60" wire:target="date,vehicleId">
                                    ...
                                    <fieldset class="mt-3">
                                        ...
                                        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                                            @foreach ($slots as $slot)
                                                ...
                                                @endphp
                                                <label @class([
                                                    'relative flex min-h-16 cursor-pointer flex-col justify-center rounded-field border px-3 py-2 text-center transition',
                                                    'border-brand bg-brand-soft text-brand' => $time === $value,
                                                    'border-warning/40 bg-warning-soft text-warning' => $time !== $value && $slot->state === 'limited',
                                                    ...
                                                    <input class="sr-only" type="radio" wire:model="time" value="{{ $value }}" @disabled(! $slot->isAvailable)>
                                ...
                                @endif
                                @error('time') <p class="mt-2 text-sm text-danger">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section class="rounded-panel border border-line bg-surface p-5 sm:p-7">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <textarea id="complaint" wire:model="complaint" rows="4" maxlength="2000" placeholder="Contoh: terdengar bunyi saat pengereman" class="mt-2 w-full rounded-field border border-line-strong bg-surface px-3.5 py-3 text-sm focus:border-brand focus:ring-2 focus:ring-brand-soft"></textarea>
                            @error('complaint') <p class="mt-1.5 text-sm text-danger">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </section>
            </div>
            ...
            <aside class="h-fit rounded-panel border border-line bg-surface p-5 lg:sticky lg:top-24">
                <p class="text-xs font-semibold uppercase tracking-[0.14em] text-ink-subtle">Konfirmasi booking</p>
